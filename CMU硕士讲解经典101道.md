...
3、==找起始点==：将环分为三段，环外长度m，环中首次相遇点之前为n，之后为k，根据首个循环有2(m+n) = m + n + k + n ->推出m = k。到这基本明了了，慢指针从相遇点出发和快指针从头出发会交到环起始点
...
```
**本题思路**：
1、先算出长度和的一半，然后双指针指向两个数组，向后移动到一半长度，记录好上一轮的数(以便最后偶数求中位数)
2、还有一个方法是先合并再直接求中位数(也是双指针
...
public:
    int findKthLargest(vector<int>& nums, int k) {
        ...
        while(l < r){
            int temp = quickSelection(nums,l,r);
            if(temp == target){
                return nums[temp];
            }
            else if(temp < target){
...
```
**本题思路**
1、快速选择和快排一样，数组排布好的话是有可能达到O(n)的，==其实还是有一个用例超时了，随机打乱数组才成功通关==，但也是很慢，这个方法只是做出来了而已，但它是O(n)的算法
...
```
**本题思路**：
1、用map进行遍历数量统计，然后再根据数量进行二次放入桶，最后由高到低遍历输出
...
public:
    string frequencySort(string s) {
        ...
        vector<vector<char>> char_bucket(max_count + 1);
        for(auto & p:counts){
            char_bucket[p.second].push_back(p.first);
        }
        string ans = "";
        for (int i = max_count; i >= 0; --i) {
            for (const char & q : char_bucket[i]) {
                for(int j = counts[q]; j > 0; j--)
                ans += q;
            }
        }
        return ans;
